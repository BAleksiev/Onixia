...
/* end template head */ ob_start(); /* template body */ ?>        <div id="content">
            ...
            <div class="page">
                <h2 class="title"><?php echo translate('Alliance');?></h2>
                <div class="spy-list">
...
{
	foreach ($_fh2_data as $this->scope['id']=>$this->scope['num'])
	{
/* -- foreach start output */
?>
                    <?php echo translate($this->readVar("ships.".(isset($this->scope["id"]) ? $this->scope["id"] : null).".name"));?> :
                    <input type="text" name="ships[<?php echo $this->scope["id"];?>]" value="0" size="5" class="<?php echo $this->scope["id"];?>"/>
                    <a href=javascript:setMaximum(<?php echo $this->scope["id"];?>,<?php echo $this->scope["num"];?>)><?php echo $this->scope["num"];?></a>
                    <br/>
                <?php 
/* -- foreach end output */
	}
...
/* -- for start output */
?>
                            <option value="<?php echo $this->scope["i"];?>0"><?php echo $this->scope["i"];?>0%</option>
                        <?php /* -- for end output */
	}
}
?>
